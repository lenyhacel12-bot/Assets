...
> This document describes the *intended* entities, their key fields, and the
...
- All arithmetic that produces a *posted* financial/stock value is performed in
  SQL / Postgres functions, never in JS floating point.

---

## Module A — Identity, branches & permissions *(Stage 2)*
...
## Module B — Master records *(Stage 3)*
...
## Module C — Inventory engine *(Stage 4)*

> **Balances are derived from immutable movements. Never edit balances.**

- **inventory_movements** *(immutable, append-only)* — `id, branch_id,
...
- **inventory_balances** *(maintained ONLY by the posting function)* —
...
## Module D — Purchasing & payables *(Stage 5)*

- **purchase_requests** *(optional flow)* — header + lines.
...
## Module E — Sales & receivables *(Stage 6)*
...
## Module F — Transfers & deliveries *(Stage 7)*
...
## Module H — Accounting engine *(Stage 9)*
...
- **journal_entries** *(immutable once posted)* — `id, branch_id, entry_date,
...
## Module I — Cash, banks & treasury *(Stage 10)*
...
## Module L — Approvals & audit *(Stage 13)*
...
- **approval_requests** — `id, type (discount/damage/transfer/refund/adjustment/
  ...
  reason`.
- **audit_logs** *(immutable)* — `id, actor_id, action, entity_type, entity_id,
  before jsonb, after jsonb, branch_id?, created_at`.

## Module M — Document numbering *(cross-cutting)*
...
## Module N — QuickBooks migration *(Stage 16)*
